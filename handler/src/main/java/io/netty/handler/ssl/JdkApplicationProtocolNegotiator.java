...
class JdkApplicationProtocolNegotiator implements ApplicationProtocolNegotiator {
    private static final boolean ALPN_AVAILABLE = ConscryptAlpnSslEngine.isAvailable()
    ...
    SSLEngine wrapSslEngine(SSLEngine engine, boolean isServer) {
        switch (protocol) {
            case NONE:
                return engine;
            case ALPN:
                if (ConscryptAlpnSslEngine.isEngineSupported(engine)) {
                    return isServer ? ConscryptAlpnSslEngine.newServerEngine(engine, this)
                            : ConscryptAlpnSslEngine.newClientEngine(engine, this);
                }
